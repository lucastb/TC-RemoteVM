...
//	Objetivo => Implementar tr�s vms com apache e conectar nelas
//	
...
//	paused, stopped, snapshotted or other things.
	...
	// O objeto sess�o tem um objeto console.
	...
	public static SharedConnection conectarServidorArquivos(String ipServidorArquivos, String pastaCompartilhada) {
		...
		// Conecta num servidor de arquivos que permite acesso p�blico.
		AuthenticationContext authenticationContext = AuthenticationContext.anonymous();
		System.out.println("Conectando � pasta compartilhada \\\\" + ipServidorArquivos + "\\" + pastaCompartilhada);
		try {
			SharedConnection sharedConnection = new SharedConnection(ipServidorArquivos, pastaCompartilhada,
					authenticationContext);
			System.out.println("Conectado � pasta compartilhada \\\\" + ipServidorArquivos + "\\" + pastaCompartilhada);
			return sharedConnection;
		} catch (IOException e) {
			System.out.println("\nN�o foi poss�vel se conectar � pasta compartilhada.");
			return null;
		}
	}
	...
	public static void listarAppliances(String ipServidorArquivos, String pastaCompartilhada) {
		...
		if (conexaoCompartilhada != null) {
			...
			System.out.println("N�o foi poss�vel listar os arquivos.");
		}
	}

	public static VirtualBoxManager conectarWS(String ipHost) {

		// Cada inst�ncia de VirtualBoxManager � um host com o VirtualBox
		...
		System.out.println("\nConectando em " + url + " com usu�rio " + usuario + " e senha " + senha + "...");
		try {
			manager.connect(url, usuario, senha);
			System.out.println("\nCliente conectado com sucesso em " + url + " com usu�rio " + usuario + " e senha "
					+ senha + ".");

		} catch (VBoxException e) {
			System.out.println("\nO cliente nao p�de conectar ao webserver " + url + " com usu�rio " + usuario
					+ " e senha " + senha + ".");
		}
	...
	/*
	 * 14.10.2019 - API do WebService do VirtualBox n�o tem um m�todo que exibe o
	 * espa�o livre em disco. public static long getEspacoLivreDiscoHost(IVirtualBox
	 * vBoxSVC) { // 08.10.2019 - esta pegando o espa�o em disco do host local, ao
	 * inv�s do remoto ISystemProperties isp = vBoxSVC.getSystemProperties();
	 ...
	 * 
	 * // System.out.println("Espa�o Livre: " + file.getFreeSpace()); // // double
	...
	public static void desconectarWS(VirtualBoxManager manager) {
		...
		try {
			...
			System.out.println("\nN�o foi poss�vel se desconectar do webservice.");
		}
	}

	// IP - online - capacidade - (margem)
	// USO FINAL = (Mem�ria utilizado + Mem�ria Appliance)/Mem�ria Total
	...
	static void ligarVM(VirtualBoxManager manager, IVirtualBox vbox, String machine) {
		...
		try {
			...
			if (m.getState().name() != "Running" || m.getState().name() != "Starting") {
				...
				System.out.println("VM " + name + " j� esta no estado " + m.getState().name() + ".");
			}
...
//		IMachine getSessionState: Locked		
	...
	static void detalharHost(IVirtualBox vBoxSVC) {
		...
		System.out.println(
				...
				"\nMem�ria total: " + h.getMemorySize() + " MB" + "\nMem�ria utilizada: "
						+ (h.getMemorySize() - h.getMemoryAvailable()) + " MB" + "\nMem�ria dispon�vel: "
						+ h.getMemoryAvailable() + " MB" + "\nUso de Mem�ria: " + Math.round(c) + " %"
						+ "\nSistema Operacional: " + h.getOperatingSystem() + "\nVers�o: " + h.getOSVersion() +
//				"\nN�cleos de Processamento: " + h.getProcessorCoreCount() +
//				"\nProcessadores L�gicos: " + h.getProcessorCount() +
//				"\nN�cleos de Processamento Online:" + h.getProcessorOnlineCoreCount() +
//				"\nProcessadores L�gicos Online: " + h.getProcessorOnlineCount()
						"\nN�cleos de Processamento: " + h.getProcessorOnlineCoreCount() + "\nProcessadores L�gicos: "
...
//	            https://github.com/OpenCyberChallengePlatform/OccpAdmin/blob/master/occp/edu/uri/dfcsc/occp/OccpVBoxHV.java
			...
			for (String s : lista) {
				...
				for (String a : arrOfStr) {
					i++;
					if (i == 2) {
						System.out.println("Capacidade M�xima do disco (Bytes): " + a);
					}
				}
			}
...
//		se nao der pra transferir, muda a arquitetura pra assumir que os arquivos j� est�o em todas as m�quinas
...
//			}
	...
	public String oberIPPeloHostname(String hostname) {
		try {
			...
			System.out.println("N�o pode resolver o endere�o IP do host " + hostname);
			return null;
		}
	}

	// o worst fit, que deixa muita mem�ria sobrando, seria, no nosso caso, o best
	// fit,
	// pois deixa mais mem�ria pro usu�rio

	// o best fit, que deixa pouca mem�ria sobrando, seria, no nosso caso, o worst
	// fit
	// pois deixa pouca mem�ria pro usu�rio

	public String getParametros() {
		return parametros;
	}

	// o first fit, no nosso caso, apenas pegaria o primeiro com mem�ria dispon�vel,
	// sem considerar a atual utiliza��o da m�quina host
	...
	public static void exibirMenuPrincipal() {
		...
		while (true) {
			...
			exibirCabecalho();
			System.out.println("\nSelecione uma op��o:");
			System.out.println("1) Adicionar m�quina(s) h�spede(s) a partir de arquivo com endere�os IPs");// importar
			System.out.println("2) Listar IPs cadastrados");
			System.out.println("2) Adicionar m�quina h�spede a partir de endere�o IP");// adicionar endere�o ip de
			System.out.println("3) Editar pool de m�quinas");
			System.out.println("4) Descrever pool de m�quina(s)");
			System.out.println("5) Exibir endere�o do servidor de arquivos");
			...
			System.out.println("");
			System.out.print("Entre o n�mero para selecionar uma op��o:\r\n");
...
//		if (args.length != 1 || args[0] == "-?") {
	...
	public static void main(String[] args) throws java.io.IOException, InterruptedException {
		...
		// objetivo = alta disponibilidade de servidores web ngix com v�rias VMS
		// Exibir console - conectar, ver as vms, ver se est� ativona VM, e obter a
		// porta
		// Copiar pros hosts sem precisar ir um por um.

		// Se n�o estiver ativo, ativar. Se a porta j� existir, substituir.
		//
