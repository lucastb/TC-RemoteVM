...
class PerformanceCollector {
	...
	public List < PerformanceData > query(List < String > filterMetrics, List < IUnknown > filterObjects) {
		...
		List < Integer > values = _collector.queryMetricsData(filterMetrics, filterObjects, names, objects, units, scales, sequenceNumbers, indices, lengths);
...
public class metrictest implements Runnable {
	...
	public metrictest() {
		mgr = new IWebsessionManager("http://localhost:18083/", null, null);
		//mgr = new IWebsessionManager();
	...
	//        	String machineName = object.getRemoteWSPort().iMachineGetName(object.getWrapped());
	//            return machineName;
	//        } catch (Exception e)
	//        {
	//        }
	...
	public void setup() {
		perf.setup(Arrays.asList(new String[] {
			"*"
		}), new ArrayList < IUnknown > (), new Long(1), new Long(5));
	}
	...
	public void collect() {
		try {
			...
			for (PerformanceData md: metricData) {
				//System.out.println("(" + getObjectName(md.object) + ") " + md.name + " " + md.getFormattedSamples());
			}
		}
		catch(Exception e) {
			e.printStackTrace();
